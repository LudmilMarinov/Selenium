		...
		if(!allWindows.isEmpty()) {
			for (String windowId : allWindows) {
				...
				if(driver.getPageSource().contains("Build my Car - Configuration - Online Chat")) {
					/* The moment exception occur in the try block - an exception object of a specific type is being
					created. Now if the type of the exception object is the same or child type of the types listed in the catch
					block or blocks --> the catch block will catch it and whatever is coded in the catch block will be executed
					and in that case it is considered that the exception was successfully caught.
					If the type is nor the same, nor a child type of the one expected by the catch block - the execution
					of the program will stop. For more reference - check the Lecture slides from the Exceptions lecture in the Java module!
					 */
					try {
